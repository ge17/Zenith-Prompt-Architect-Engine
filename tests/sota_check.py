...
def verify_sota():
    ...
    try:
        ...
        # 3. Check Sub-modules
        if not agent.analyzer: raise ValueError("Analyzer missing")
        if not agent.validator: raise ValueError("Validator missing")
        if not agent.judge: raise ValueError("Judge missing")
        if not agent.knowledge_base: raise ValueError("KnowledgeBase missing")
        if not agent.main_session: raise ValueError("Main Session missing")
