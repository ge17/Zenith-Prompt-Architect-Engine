...
def mock_agent_deps():
    with (
        patch("src.core.agent.StrategicAnalyzer") as MockAnalyzer,
...
async def test_run_analysis_flow(mock_config, mock_agent_deps):
    ...
    mock_agent_deps["analyzer"].return_value.analyze_intent_async = AsyncMock(
        return_value={"natureza": "Raciocínio", "complexidade": "Simples"}
    )

    mock_agent_deps["kb"].return_value.retrieve_async = AsyncMock(
    ...
    # Mock Judge (Pass)
    mock_agent_deps["judge"].return_value.evaluate_async = AsyncMock(
        return_value={"score": 90, "needs_refinement": False}
    )
